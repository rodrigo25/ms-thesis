function [Models, ModelosFinal] = trainModelsCocluster(biClustResult, eegReal, cluster_por_pessoa)
  ...
  for i_clust = 1:biClustResult.ClusterNo % Iteração passando por cada co-cluster
    ...
    for fold = 1:eegReal.c.NumTestSets  % Fold do Cross-Validation      
      ...
      svmParameters = ['-q -c 180 -g 0.07'];
      Models{i_clust,fold} = svmtrain(Ytrain,Xtrain,svmParameters); % Treina modelo com dos dados do treinamento do fold atual
    end
    ...
    svmParameters = ['-q -c 180 -g 0.07'];
    ModelosFinal{i_clust,1} = svmtrain(Ytrain,Xtrain,svmParameters); % Treina modelo com dos dados do treinamento do fold atual
