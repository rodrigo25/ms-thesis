function [Models, ModelosFinal] = trainModelsCoclusterELM(biClustResult, eegReal, cluster_por_pessoa)
  ...
  for i_clust = 1:biClustResult.ClusterNo % Iteração passando por cada co-cluster
    ...
    for fold = 1:eegReal.c.NumTestSets  % Fold do Cross-Validation      
      ...
      y_binary = transformClassLabels2Binary(Ytrain);
      ...
      Models{i_clust,fold} = trainELM(Xtrain,y_binary(ind_rows,:),nh); % Treina modelo com dos dados do treinamento do fold atual
    ...
    y_binary = transformClassLabels2Binary(eegReal.y);
    y_binary = y_binary(ind_rows,:);
    
    %ModelosFinal{i_clust,1} = fitcknn(Xtrain,Ytrain,'NumNeighbors',1,'Distance','euclidean'); 
    ModelosFinal{i_clust,1} = trainELM(Xtrain,y_binary,nh); % Treina modelo com dos dados do treinamento do fold atual
  end
end