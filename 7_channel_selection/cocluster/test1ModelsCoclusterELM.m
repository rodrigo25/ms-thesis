function [AccTest, AccDadosImag] = test1ModelsCoclusterELM(biClustResult, eegReal, eegImag, models, modeloFinal, cluster_por_pessoa)
  ...
  [YY] = transformClassLabels2Binary(eegReal.y);
  ...
  for i_fold = 1:eegReal.c.NumTestSets  % Fold do Cross-Validation    
    ...
    y_binary = transformClassLabels2Binary(Yfold);
    ...
    for i_clust=1:biClustResult.ClusterNo
      ...
      Y = Yfold(ind_rows,:);
      %YY = y_binary(ind_rows,:);
      Ygeral = [Ygeral; Y];
      
      %Yresult = predict(models{i_clust,i_fold},X);  % Prediz rotulos dos dados imaginados
      [Yresult, ~] = predictELM(models{i_clust,i_fold},X);  % Prediz rotulos dos dados de teste do fold atual
      [Yresult] = transformClassLabels2Decimal(Yresult);
  ...
  for i_clust=1:biClustResult.ClusterNo
    ...
    %YY = transformClassLabels2Binary(eegImag.y(ind_rows,:));
    Ygeral = [Ygeral; Y];
    
    %Yresult = predict(modeloFinal{i_clust},X);  % Prediz rotulos dos dados imaginados
    [Yresult, ~] = predictELM(modeloFinal{i_clust},X);  % Prediz rotulos dos dados de teste do fold atual
    Yresult2 = [Yresult2; Yresult];
  end
  [Yresult2] = transformClassLabels2Decimal(Yresult2);
